...
from path_def import execl_modify_path_def
import shutil
### ========
# PATH DEFINE
RESULT_ID,WRITE_PATH, WRITE_PATH_ANALYZE,WRITE_PATH_MODIFY= execl_modify_path_def()
###========
# 從ANALYZE資料夾複製csv和excel過來

for name in ['雨滴每秒結果','雨滴分鐘結果','雨滴小時結果']:
  src=WRITE_PATH_ANALYZE + name+ RESULT_ID
  dst=WRITE_PATH_MODIFY  + name+ RESULT_ID
  shutil.copyfile(src+'.csv',dst+'.csv')
  shutil.copyfile(src+'.xlsx',dst+'.xlsx')


df = pd.read_excel(WRITE_PATH_MODIFY+'雨滴每秒結果'+RESULT_ID+'.xlsx',index_col=0)
#df = pd.read_csv(PATH_ROOT +'/Raindrop_folder/Rainfall_project_2023/result/result_20231205_indoor-2/analyze_20231205_indoor-2/雨滴每秒結果20231205_indoor-2.csv',index_col=0)
deltapc=0.0
for i,v in enumerate(df['速度'][:]):
    deltapc=v*1000*12.3*(69e-6) #500=3.35
    new_long=df['長軸'][i]-deltapc
    df['長軸'][i],df['短軸'][i]= ((df['短軸'][i],new_long) if new_long< df['短軸'][i] else (new_long,df['短軸'][i]))
    if(df['長軸'][i]<0 or df['短軸'][i]<0):
      df.drop(i,inplace=True)
      continue
    old_diameter=df['直徑'][i]
    df['直徑'][i]=(8.13e-2)*(((df['長軸'][i]**2)*df['短軸'][i])**(1 / 3))
    calibrating_factor=df['直徑'][i]/old_diameter
    
df.to_excel(WRITE_PATH_MODIFY+'雨滴每秒結果'+RESULT_ID+'.xlsx',index=False)
df.to_csv(WRITE_PATH_MODIFY+'雨滴每秒結果'+RESULT_ID+'.csv',index=False)
...
with open( WRITE_PATH_MODIFY + '雨滴分鐘結果' + RESULT_ID + '.csv', newline='',encoding="utf-8") as csv1:
    rain_minute = list(csv.reader(csv1))
    rain_minute=np.array(rain_minute)
    rain_minute=rain_minute[1:]

with open( WRITE_PATH_MODIFY + '雨滴小時結果' + RESULT_ID + '.csv', newline='',encoding="utf-8") as csv2:
    rain_hours = list(csv.reader(csv2))
    rain_hours=np.array(rain_hours)
    rain_hours=rain_hours[1:]

with open( WRITE_PATH_MODIFY+ '雨滴每秒結果' + RESULT_ID + '.csv', newline='',encoding="utf-8") as csv3:
...
for i in range(0,len(tmp_rainfal_minh)): #tmp_rainfal_minh: 時間 雨量 數量
    ...
    plt.savefig(WRITE_PATH_MODIFY + RESULT_ID +'_數量(第{}小時).png'.format(i+strat_index))
    ...
    plt.savefig(WRITE_PATH_MODIFY + RESULT_ID + '_雨量(第{}小時).png'.format(i+strat_index))
# -----------------------------------------------------
...
for i in range(0,len(tmpdatah)):
    ...
    with open(WRITE_PATH_MODIFY+ RESULT_ID + '_速度對直徑H{}.csv'.format(i+strat_index),'w',newline='',encoding="utf-8") as f1:
        writer=csv.writer(f1)
        writer.writerow(X)
        for j in range(0,len(tmpdatahsum[i])):
            writer.writerow(np.insert(tmpdatahsum[i][j],0,Y1[j]))
        writer.writerow(X)
    #csv_to_xlsx_pd(WRITE_PATH_MODIFY+ ID +'_速度對直徑H{}'.format(i+strat_index))
    csv_to_xlsx_pd('_速度對直徑H{}'.format(i+strat_index), WRITE_PATH_MODIFY+ RESULT_ID)
# ------------------------------------------csv
# Y1=Y[::-1] row
# X           colum
...
for i in range(0,len(tmpdatahsum2)):
    ...
    plt.grid()
    # plt.show()
    plt.savefig(WRITE_PATH_MODIFY + RESULT_ID + '_雨量&速度&直徑(第{}小時).png'.format(i+strat_index))
# ------------------------------------------heatmap
