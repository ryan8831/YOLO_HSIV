...
def match_raindrop(input_data, latter_len, sess, SIGMOID_THRESHOLD,input_data_
    ...
    if input_data.shape[0] != 0:
        ...
        mask = (input_data_[:, 2] > 1000) | (input_data_[:, 5] > 1000)
        random_forest_data = input_data_[mask]
        actived_y = sess.run(sess_y, feed_dict = {X: input_data, k_prob: 0, tst: True})
        if random_forest_data.size > 0:
            ...
            new_input_data = []
            for row in random_forest_data:
                x1, y1, area1, x2, y2, area2, velocity = row
                # 從字典中查找對應的長短軸
                axes1 = axis_mapping.get((x1, y1, area1))
                axes2 = axis_mapping.get((x2, y2, area2))
                new_row = [x1, y1, area1, axes1[1],axes1[0], x2, y2, area2, axes2[1],axes2[0], velocity]
                new_input_data.append(new_row)
            random_forest_data = np.array(new_input_data)  
            features = pd.DataFrame(random_forest_data, columns=['x1', 'y1', 'area1', 'a1', 'b1', 'x2', 'y2', 'area2', 'a2', 'b2', 'velocity'])
...
def makeANN_label_data(former_data, latter_data,former_ellipse_axis,latter_ellipse_axis):
    output = np.array([], dtype = np.float64).reshape(0, 10)
    former_data_ax=np.hstack((former_data, former_ellipse_axis))
    latter_data_ax=np.hstack((latter_data, latter_ellipse_axis))
    former_len =  former_data_ax.shape[0]
    latter_len = latter_data_ax.shape[0]  #
    if former_len != 0 and latter_len != 0:
        sub_data = latter_data_ax[:, 0 : 5].reshape(-1, 5)    # @@? why reshape
        for i in range(former_len):
            main_data = np.matlib.repmat(former_data_ax[i][0 : 5].reshape(1, 5), latter_len, 1)
            # print( np.hstack((main_data, sub_data)).shape)
            # print(output.shape)
            output = np.vstack((output, np.hstack((main_data, sub_data))))#前後兩張圖雨滴的排列組合
    velocity = (8.13e-5)*np.sqrt((output[:, 0] - output[:, 5])**2 + (output[:, 1] - output[:, 6])**2)*500    # @@? 8.13e-5 ??
    output = np.hstack((output, velocity.reshape(-1, 1)))
    return output
def make_label_data(former_data, latter_data,former_ellipse_axis,latter_ellipse_axis):
    output = np.array([], dtype = np.float64).reshape(0, 10)
    former_data_ax=np.hstack((former_data, former_ellipse_axis))
    latter_data_ax=np.hstack((latter_data, latter_ellipse_axis))
    former_len =  former_data_ax.shape[0]
    ...
    output = np.hstack((output, velocity.reshape(-1, 1)))
    return output
def yolo_bounding_para(latter_img, closing_kernel,latter_name):
    ...
    for i in range(1, (label[1] + 1)):
        ...
        rectcx,rectcy=int(rect[0][0]),int(rect[0][1])
        rectw,recth=int(rect[1][0]),int(rect[1][1])
        xcenter=round(rectcx/640,6)
        ycenter=round(rectcy/480,6)
        yolo_w=round(rectw/640,6)
        yolo_h=round(recth/480,6)
        labeldata.append([f"{xcenter:.6f}",f"{ycenter:.6f}",f"{yolo_w:.6f}",f"{yolo_h:.6f}"])
    return labeldata
def yolo_bounding(ori_img,yolov9det):
    output = np.array([], dtype=np.float64).reshape(0, 3)
    ellipse_axis = np.array([], dtype=np.float64).reshape(0, 2)
...
for latter_name in file_list:
    ...
    if latter_name[0] == KEY_CHAR and int(latter_name[18 : 21]) - int(former_name[18 : 21]) == 1:
        ...
        latter_img=cv2.imread(READ_PATH + latter_name)
        latter_img = np.transpose(latter_img, axes = (1, 0, 2))   # transpose函數里，軸默認順序是z，x，y，分別對應0軸，1軸和2軸
        ...
        have_rain=any(l_diameters >=1.25) or any(f_diameters>=1.25)
        new_number = str(int(latter_name[18 : 21]) - 1)
        ori_file_name=latter_name[:18]+new_number.zfill(3)+latter_name[21:]
        yolo_labeldata[ori_file_name]=yolo_bounding_para(latter_img, closing_kernel,latter_name)
        # ANNlabeldata_=make_label_data(former_data, latter_data,former_ellipse_axis,latter_ellipse_axis)
        # for i in range(len(ANNlabeldata_)):
        #     f_d=np.digitize(calculate_diameter(ANNlabeldata_[i][3:5].reshape(1, -1)),rainclass)
        #     l_d=np.digitize(calculate_diameter(ANNlabeldata_[i][8:10].reshape(1, -1)),rainclass)
        #     ANNlabeldata.append([compose_img_name,f"{f_d[0]}--->{l_d[0]}",ANNlabeldata_[i][0],ANNlabeldata_[i][1],ANNlabeldata_[i][2],
        #                 ANNlabeldata_[i][3],ANNlabeldata_[i][4],
        #                 ANNlabeldata_[i][5],ANNlabeldata_[i][6],ANNlabeldata_[i][7],
        #                 ANNlabeldata_[i][8],ANNlabeldata_[i][9],
        #                 ANNlabeldata_[i][10]])
        data=[]
        ...
        if (SAVE_ARR_IMG and have_rain):
            #matched_pairindex = np.where(np.logical_or(matched_pair[:, 2] > 900, matched_pair[:, 5] > 900))[0]
            #matched_pair = matched_pair[matched_pairindex]
        ...
        former_name = latter_name
        former_img=cv2.imread(READ_PATH + former_name)
        former_img = np.transpose(former_img, axes = (1, 0, 2))
        former_data, former_ellipse_axis, _, _ = bounding_para(former_img, closing_kernel)
        #former_ellipse_axis = np.zeros((1, 2))
    else:
        # give a fake name
        former_name = '9'*25
...
with open( WRITE_PATH_ANALYZE +'cost.csv','w',newline='', encoding='UTF-8') as costfile:
    writer=csv.writer(costfile)
    writer.writerow(["composimgname","formercx","formercy","formerarea","latercx","latercy","laterarea","COST","cost_range"])
    writer.writerows(rainfall_agent.costdata)
with open( WRITE_PATH_ANALYZE +'ANNlabeldata.csv','w',newline='', encoding='UTF-8') as labelfile:
    writer=csv.writer(labelfile)
    writer.writerow(["compose_img_name","f--->l","former_data_x","former_data_y","former_data_area",
                     "former_ellipse_axis_s","former_ellipse_axis_l",
                     "latter_data_x","latter_data_y","latter_data_area",
                     "latter_ellipse_axis_s","latter_ellipse_axis_l",
                    "velocity"])
    writer.writerows(ANNlabeldata)
for filename, data in yolo_labeldata.items():
    txtfile=filename.replace('.png', '.txt')
    txtfile='o'+txtfile[1:]
    label_path=WRITE_PATH_ANALYZE+'label/'
    if not os.path.exists(label_path):
        os.makedirs(label_path)
    output_path = os.path.join(label_path,txtfile )
    with open(output_path, 'w') as file:
        for item in data:
            line = '1 ' + ' '.join(map(str, item)) 
            file.write(line + '\n')
...
print("\n=========================================================================================================\n") 
check_match(WRITE_PATH,WRITE_PATH_ANALYZE,WRITE_PATH_ARRIMG)
print("  !!!!!!  PROCESS ARR DEBUG IMG !!!!!!  \n")
#get_rainfall_ARR(RESULT_ID, WRITE_PATH, WRITE_PATH_ANALYZE,WRITE_PATH_ARRIMG)
# =========================================================================================================
